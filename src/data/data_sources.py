...
def _resolve_optional_file(candidates: List[str]):
...
def _attach_ogbl_ppa_external_node_embeddings(
    graph: Data, data_dir: str, data_path: str = ""
):
    """
    Optionally attach precomputed node embeddings to ogbl-ppa.

    Expected files:
      - node_x_embed.pt: [num_nodes, embed_dim]
      - node_has_esm.pt: [num_nodes] (optional)

    Search order:
      1. env GRAPHGPT_OGBL_PPA_NODE_EMBED_PATH / GRAPHGPT_OGBL_PPA_NODE_MASK_PATH
      2. <data_path>/node_x_embed.pt if `data_path` is a directory
      3. <cwd>/new/esm_outputs/node_x_embed.pt
    """
    embed_candidates = [
        os.environ.get("GRAPHGPT_OGBL_PPA_NODE_EMBED_PATH", ""),
        os.path.join(data_path, "node_x_embed.pt") if os.path.isdir(data_path) else "",
        os.path.join(os.getcwd(), "new", "esm_outputs", "node_x_embed.pt"),
    ]
    mask_candidates = [
        os.environ.get("GRAPHGPT_OGBL_PPA_NODE_MASK_PATH", ""),
        os.path.join(data_path, "node_has_esm.pt") if os.path.isdir(data_path) else "",
        os.path.join(os.getcwd(), "new", "esm_outputs", "node_has_esm.pt"),
    ]

    embed_path = _resolve_optional_file(embed_candidates)
    if embed_path is None:
        print(
            "[ogbl-ppa] No external node embedding file found. "
            "Skip attaching `graph.x_embed` and keep the original pipeline."
        )
        return graph

    x_embed = torch.load(embed_path, map_location="cpu")
    if not isinstance(x_embed, torch.Tensor):
        raise TypeError(
            f"Expected tensor from node embedding file, got {type(x_embed)} from {embed_path}"
        )
    if x_embed.ndim != 2:
        raise ValueError(
            f"Expected x_embed to be 2D [num_nodes, embed_dim], got shape {tuple(x_embed.shape)}"
        )
    if x_embed.shape[0] != graph.num_nodes:
        raise ValueError(
            f"External node embedding row count mismatch: {x_embed.shape[0]} != {graph.num_nodes}"
        )

    graph.x_embed = x_embed.to(torch.float32)

    mask_path = _resolve_optional_file(mask_candidates)
    if mask_path is not None:
        has_esm = torch.load(mask_path, map_location="cpu")
        if not isinstance(has_esm, torch.Tensor):
            raise TypeError(
                f"Expected tensor from node mask file, got {type(has_esm)} from {mask_path}"
            )
        if has_esm.ndim != 1:
            has_esm = has_esm.view(-1)
        if has_esm.shape[0] != graph.num_nodes:
            raise ValueError(
                f"External node mask length mismatch: {has_esm.shape[0]} != {graph.num_nodes}"
            )
        graph.has_esm = has_esm.to(torch.bool)
        coverage = graph.has_esm.float().mean().item()
        print(
            f"[ogbl-ppa] Loaded external node embeddings from {embed_path}\n"
            f"[ogbl-ppa] Loaded node coverage mask from {mask_path}\n"
            f"[ogbl-ppa] ESM coverage: {coverage:.6f}"
        )
    else:
        print(
            f"[ogbl-ppa] Loaded external node embeddings from {embed_path}\n"
            "[ogbl-ppa] node_has_esm.pt not found, skip attaching `graph.has_esm`."
        )
    return graph


def _load_local_ogbl_ppa_graph_and_split(data_dir: str, data_path: str):
    dataset_root = os.path.join(data_dir, data_path)
    graph_file = os.path.join(dataset_root, "graph.pt")
    split_file = os.path.join(dataset_root, "split_edge.pt")
    if not os.path.isfile(graph_file):
        raise FileNotFoundError(
            f"Cannot find local ogbl-ppa subgraph graph file: {graph_file}"
        )
    if not os.path.isfile(split_file):
        raise FileNotFoundError(
            f"Cannot find local ogbl-ppa subgraph split file: {split_file}"
        )
    graph = torch.load(graph_file, map_location="cpu", weights_only=False)
    split_edge = torch.load(split_file, map_location="cpu", weights_only=False)
    if not hasattr(graph, "x"):
        raise ValueError(
            f"Expected local ogbl-ppa subgraph graph to have `x`, got keys: {graph.keys()}"
        )
...
def _load_ogb_linkprop_dataset_and_split(data_dir: str, dataset_name: str):
    with _force_torch_load_weights_only_false():
        raw_dataset = PygLinkPropPredDataset(root=data_dir, name=dataset_name)
        split_edge = raw_dataset.get_edge_split()
    return raw_dataset, split_edge


def _read_local_ogbl_ppa_like_dataset(
    data_cfg: DataConfig,
    *,
    ...
    graph, split_edge, dataset_root = _load_local_ogbl_ppa_graph_and_split(
        data_dir, data_path
    )
    print(
        f"Loading {dataset_name} local subgraph data from dir: {dataset_root}\n"
        f"graph -> {graph}"
    )
    ...
    if return_valid_test:
        ...
        print(
            f"Split dataset based on local train/valid/test index!\n"
...
def _read_ogbl_ppa(data_cfg: DataConfig, *, train_cfg: TrainingConfig, **kwargs):
    ...
    print(f"Loading {dataset_name} raw data from dir: {data_dir}")
    raw_dataset, split_edge = _load_ogb_linkprop_dataset_and_split(
    ...
    graph.x_gid = _get_global_local_id_from_onehot(graph.x, global_id_only=True)
    graph.x = _get_global_local_id_from_onehot(graph.x, global_id_only=False)
    graph = _attach_ogbl_ppa_external_node_embeddings(graph, data_dir, data_path)
    print(
        f"\nLoading dataset from {graph} with ShaDowKHopSeqFromEdgesMapDataset.\nParams:\nsampling_config: {pformat(sampling_config)}"
    )
    if return_valid_test:
...
@_dataset("ogbl-ppa-human")
def _read_ogbl_ppa_human(data_cfg: DataConfig, *, train_cfg: TrainingConfig, **kwargs):
    return _read_local_ogbl_ppa_like_dataset(
        data_cfg,
        train_cfg=train_cfg,
        dataset_name="ogbl-ppa-human",
    )
