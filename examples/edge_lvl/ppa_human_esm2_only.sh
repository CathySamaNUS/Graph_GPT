#!/bin/bash
# ===========================================================================
# Experiment 2: ESM-2 Only (no graph discrete features)
# PCA-reduced ESM-2 embeddings (256-dim from 1280-dim, 95.3% variance)
# Model: tiny (h128, l2), 6 epochs
# ===========================================================================
...
WARMUP_EPOCHS=0.6
EMBED_DIM=256
OUTPUT_DIR="./exp/models/ogbl_ppa_human/esm2only_pca256_h128_l2_b128_e6_lr3e-4"

echo "========================================"
echo "Experiment 2: ESM-2 Only (PCA 256-dim)"
echo "Model: tiny h=${HIDDEN} l=${LAYERS}"
echo "ESM-2: PCA 1280->256 (95.3% variance)"
echo "Coverage: 95.5%"
echo "Discrete node features: DISABLED"
...
python ./examples/train_supervised.py \
  ...
  tokenization.semantics.node.dim=0 \
  'tokenization.semantics.node.discrete=null' \
